<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Contactanos</title>
    <link rel="stylesheet" href="../bootstrap/css/bootstrap.min.css">
<<<<<<< HEAD
    <link rel="stylesheet" href="../assets/css/globales.css">
    <link rel="stylesheet" href="../assets/css/header.css">
    <link rel="stylesheet" href="../assets/css/alerts.css">
    <link rel="stylesheet" href="../assets/css/Contacto.css">
    <style>
  
    </style>
</head>
<body>
    <?php 
    include "../includes/header-contacto1.php";
    ?>
    <div class="col-lg-12 d-flex justify-content-center align-items-center vh-100">
        <div class="container p-4 rounded shadow" style="max-width: 400px;">
            <div class="text-center mb-4">
                <h1 class="h3 mb-4">¿Quieres contactarnos?</h1>
            </div>
            <div class="mb-3">
                <img src="data:image/png;base64," width="35" height="35">
                <a class="texto_contacto" href="https://wa.me/2494662744">WhatsApp</a>
            </div>
            <div class="mb-3">
                <img src="data:image/png;base64," width="35" height="35">
                <a class="texto_contacto" href="https://www.instagram.com/">Instagram</a>
            </div>
            <div class="mb-3">
                <img src="data:image/png;base64," width="35" height="35">
                <a class="texto_contacto" href="https://www.facebook.com/">Facebook</a>
            </div>
            <div class="text-center mt-4">
                <p>¡Estamos aquí para ayudarte!</p>
            </div>
        </div>
    </div>
    <script src="../bootstrap/js/bootstrap.bundle.min.js"></script>
</body>
</html>
=======
     <link rel="stylesheet" href="../assets/css/Estil.css">
    
</head>
<body>
    <?php 
    include "../includes/header-contacto.php";
     ?>
    <div class="col-lg-12 d-flex justify-content-center align-items-center vh-100">
        <div class="container bg-dark text-light p-4 rounded shadow" style="max-width: 400px;">
            <div class="text-center mb-4">
        <h1 class="h3 mb-4">¿Quieres contactarnos?</h1>
            </div>
                <div class="mb-3">
                    <img src="data:image/png;base64," width="35" height="35" >
                    <a class="texto_contacto" href="https://wa.me/2494662744">Whatssapp</a>
                </div>
                <div class="mb-3">
                    <img src="data:image/png;base64," width="35" height="35" >
                    <a class="texto_contacto" href="https://www.instagram.com/">Instagram</a>
                </div>
                <div class="mb-3">
                    <img src="data:image/png;base64," width="35" height="35" >
                    <a class="texto_contacto" href="https://www.facebook.com/reel/863930885156363">Facebook</a>
                </div>
            </div>    
        </div>
    </body>
</html>
>>>>>>> a0c53c1ba67e5c1aed1917f2be81ce77883a9a92
