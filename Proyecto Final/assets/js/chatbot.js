async function FetchOpenAIResponse() {
    ...
    const context = `
        ...
        Si no puedes sugerir una receta con los ingredientes dados, indica que falta algún ingrediente adicional para hacer una receta viable. Podes decir sal o azucar a gusto segun la receta lo requiera.
    ...
    `;
    const context2 = `
        Si los ingredientes dados no combinan para una receta común, puedes ajustar o simplificar, pero deja claro que estás eliminando ingredientes que no encajan. Si te hacen una pregunta no relacionada con cocina, di que no fuiste diseñado para eso, pero si la pregunta es una duda del usuario respondela siempre del ambito de la cocina y si te pregunta tu nombre di que eres RecipeEase.
    `;
    ...
    try {
        ...
        // Mostrar el prompt del usuario en la sección correspondiente
        document.getElementById('user-prompt-display').textContent += `Tu pregunta: ${userPrompt}\n`;
    
        // Limpiar el campo de entrada
        document.getElementById('userPrompt').value = '';
    
        // Aquí podrías hacer la llamada a la API de OpenAI, pero por ahora lo omitimos
    }
